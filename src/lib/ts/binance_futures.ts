import type { binanceMarket, exchangeFutureDataT } from 'src/app';
...
export async function get_binance_futs() {
	...
	const perp_markets: exchangeFutureDataT[] = p_json.reduce(
		(result: exchangeFutureDataT[], element: binanceMarket) => {
	...
	const dated_markets: exchangeFutureDataT[] = d_json.reduce(
		(result: exchangeFutureDataT[], element: binanceMarket) => {
			const maybe_date = convert_binance_shitty_expiry_string_to_timestamp(
				element.symbol.slice(-6)
			);
			if (maybe_date) {
				result.push({
					exchange: 'binance',
